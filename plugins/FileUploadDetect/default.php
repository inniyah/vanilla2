...
// Define the plugin:
$PluginInfo['FileUploadDetect'] = array(
   'Name' => 'File Upload Detect',
   'Description' => 'This plugin enables file uploads and attachments to discussions and comments. It will display the attachments with the relevant graphic based on file extension',
   'Version' => '0.2b',
...
class FileUploadDetectPlugin extends Gdn_Plugin {
   /** @var array */
   ...
   public function Controller_Toggle($Sender) {
      $FileUploadStatus = Gdn::Config('Plugins.FileUploadDetect.Enabled', FALSE);
      ...
      if ($Sender->Form->AuthenticatedPostBack()) {
         ...
         SaveToConfig('Plugins.FileUploadDetect.Enabled', $FileUploadStatus);
      }
    ...
    * 
    * Calls FileUploadDetectPlugin::PrepareController
    *
    * @access public
    * @param mixed $Sender The hooked controller
    * @see FileUploadDetectPlugin::PrepareController
    * @return void
    */
    ...
    *
    * Calls FileUploadDetectPlugin::DrawAttachFile
    * 
    * @access public
    * @param mixed &$Sender
    * @see FileUploadDetectPlugin::DrawAttachFile
    * @return void
    */
...
//            $PlacementStatus = $this->PlaceMedia($Media, Gdn::Session()->UserID);
   ...
   protected function PlaceMedia(&$Media, $UserID) {
      $NewFolder = FileUploadDetectPlugin::FindLocalMediaFolder($Media->MediaID, $UserID, TRUE, FALSE);
      $CurrentPath = array();
      ...
      $NewFilePath = FileUploadDetectPlugin::FindLocalMedia($Media, FALSE, TRUE);
   ...
   public static function FindLocalMedia($Media, $Absolute = FALSE, $ReturnString = FALSE) {
      $ArrayPath = FileUploadDetectPlugin::FindLocalMediaFolder($Media->MediaID, $Media->InsertUserID, $Absolute, FALSE);
   ...
   public function PostController_Upload_Create($Sender) {
      ...
      try {
         if (!$this->CanUpload) 
            throw new FileUploadDetectPluginUploadErrorException("You do not have permission to upload files",11,'???');
      
         if (!$Sender->Form->IsPostBack()) {
            $PostMaxSize = ini_get('post_max_size');
            throw new FileUploadDetectPluginUploadErrorException("The post data was too big (max {$PostMaxSize})",10,'???');
         }
      
         if (!$FileData) {
            //$PostMaxSize = ini_get('post_max_size');
            $MaxUploadSize = ini_get('upload_max_filesize');
            //throw new FileUploadDetectPluginUploadErrorException("The uploaded file was too big (max {$MaxUploadSize})",10,'???');
            throw new FileUploadDetectPluginUploadErrorException("No file data could be found in your post",10,'???');
         }
         ...
         if ($FileErr != UPLOAD_ERR_OK) {
            ...
            throw new FileUploadDetectPluginUploadErrorException($ErrorString, $FileErr, $FileName, $FileKey);
         }
         ...
         if (!in_array($Extension, $AllowedExtensions) && !in_array('*',$AllowedExtensions))
            throw new FileUploadDetectPluginUploadErrorException("Uploaded file type is not allowed.", 11, $FileName, $FileKey);
         ...
         if ($FileSize > $MaxUploadSize) {
            ...
            throw new FileUploadDetectPluginUploadErrorException($Message, 11, $FileName, $FileKey);
         }
         ...
         if (!$Handled) {
            ...
            if (!is_dir(dirname($SavePath)))
               throw new FileUploadDetectPluginUploadErrorException("Internal error, could not save the file.", 9, $FileName);
            
            // Move to permanent location
            $MoveSuccess = @move_uploaded_file($FileTemp, $SavePath);
            if (!$MoveSuccess)
               throw new FileUploadDetectPluginUploadErrorException("Internal error, could not move the file.", 9, $FileName);            
         }
...
//               'uploads/'
//               .FileUploadDetectPlugin::FindLocalMediaFolder($MediaID, Gdn::Session()->UserID, FALSE, TRUE)
//               .'/'
//               .$MediaID
//               .'.'
...
//         }
      ...
      } catch (FileUploadDetectPluginUploadErrorException $e) {
   ...
   protected function TrashFile($MediaID) {
      ...
      if ($Media) {
         ...
         if (!$Deleted) {
            $CalcPath = FileUploadDetectPlugin::FindLocalMedia($Media, TRUE, TRUE);
   ...
   public function Setup() {
      $this->Structure();
      SaveToConfig('Plugins.FileUploadDetect.Enabled', TRUE);
   }
   ...
   public function OnDisable() {
      RemoveFromConfig('Plugins.FileUploadDetect.Enabled');
   }
}

class FileUploadDetectPluginUploadErrorException extends Exception {
